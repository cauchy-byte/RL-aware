...
class Parameter:
    ...
    def apply_vars(self, args):
        ...
        if hasattr(args, 'rnn_fix_length') and hasattr(args, 'ep_state_history_length'):
            if args.rnn_fix_length is not None and args.rnn_fix_length > 0 and args.ep_state_history_length != args.rnn_fix_length:
                self.info(f"[WARN] rnn_fix_length({args.rnn_fix_length}) != ep_state_history_length({args.ep_state_history_length}). "
                          f"Recommended to set them equal for fixed-length EP training.")
    ...
    def parse(self):
        ...
        self.alpha_min = 0.0
        parser.add_argument('--alpha_min', type=float, default=self.alpha_min, metavar='N',
                            help='floor for the auto-tuned SAC temperature alpha; 0 disables the floor. '
                                 'Set >0 (e.g. 0.02) to prevent entropy collapse in non-stationary/delayed envs')
        self.register_param('alpha_min')
        ...
        # 动作历史长度H：RNN输入将使用最近H个动作
        self.action_history_length = 6
        parser.add_argument('--action_history_length', type=int, default=self.action_history_length, metavar='N',
                            help="number of recent actions to concatenate with state for RNN input (default: 3)")
        self.register_param('action_history_length')

        self.ep_state_history_length = 8
        parser.add_argument('--ep_state_history_length', type=int, default=self.ep_state_history_length, metavar='N',
                            help="state history length H for EP sliding window input")
        self.register_param('ep_state_history_length')

        self.ep_action_window_length = 10
        parser.add_argument('--ep_action_window_length', type=int, default=self.ep_action_window_length, metavar='N',
                            help="action window length T for EP sliding window input")
        self.register_param('ep_action_window_length')
        ...
        self.use_wmcl = False
        parser.add_argument('--use_wmcl', action='store_true', default=self.use_wmcl,
                            help="use Wasserstein Metric Context Loss to align embedding distance with delay distribution Wasserstein distance")
        self.register_param('use_wmcl')

        self.wmcl_weight = 1
        parser.add_argument('--wmcl_weight', type=float, default=self.wmcl_weight, metavar='N',
                            help="loss ratio of the Wasserstein Metric Context Loss")
        self.register_param('wmcl_weight')
        ...
        self.minimal_repre_rp_size = 5e4
        ...
        self.varying_params = []  # 默认为空，不变化物理参数
        parser.add_argument('--varying_params', nargs='*', type=str, default=self.varying_params,
                           help="physical parameters to vary (empty for none)")
        self.register_param('varying_params')

        # Delay environment parameters
        self.use_delay = True  # 默认启用延迟环境
        parser.add_argument('--use_delay', type=bool, default=self.use_delay,
        ...
        parser.add_argument('--initial_delay_type', type=str, default=self.initial_delay_type,
                            ...
                            help="initial delay distribution type")
        self.register_param('initial_delay_type')

        self.delay_changing_period = 1000
        parser.add_argument('--delay_changing_period', type=int, default=self.delay_changing_period,
                            help="period for delay distribution change")
        self.register_param('delay_changing_period')

        self.delay_changing_interval = 100
        parser.add_argument('--delay_changing_interval', type=int, default=self.delay_changing_interval,
                            help="interval between delay distribution changes")
        self.register_param('delay_changing_interval')

        self.delay_type_changing_period = 10000
        parser.add_argument('--delay_type_changing_period', type=int, default=self.delay_type_changing_period,
                            help="period for delay type (delay_type) change; set to 0 to disable")
        self.register_param('delay_type_changing_period')

        self.max_delay_range_min = 3
        parser.add_argument('--max_delay_range_min', type=int, default=self.max_delay_range_min,
                            help="minimum value of max delay range")
        self.register_param('max_delay_range_min')

        self.max_delay_range_max = 10
        parser.add_argument('--max_delay_range_max', type=int, default=self.max_delay_range_max,
                            help="maximum value of max delay range")
        self.register_param('max_delay_range_max')

        self.nonstationary_delay = True  # 默认启用非平稳延迟
        parser.add_argument('--nonstationary_delay', type=bool, default=self.nonstationary_delay,
                            help="use nonstationary delay (delay changes during training)")
        self.register_param('nonstationary_delay')

        self.random_delay_per_episode = True  # 默认每个episode随机延迟
        parser.add_argument('--random_delay_per_episode', type=bool, default=self.random_delay_per_episode,
                            help="randomize delay parameters at the start of each episode")
        self.register_param('random_delay_per_episode')

        self.delay_task_num = 10
        parser.add_argument('--delay_task_num', type=int, default=self.delay_task_num,
                            help="number of delay tasks for meta-learning")
        self.register_param('delay_task_num')
