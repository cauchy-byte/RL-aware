...
def visualize_repre(data, valid, output_file, real_param_dict=None, tasks=None):
    ...
    for ind, item in enumerate(data):
        ...
        if real_param_dict is None:
            ...
            task_num = tasks[ind]
             # 归一化 task_num 的类型，避免 np.float32 / torch scalar / float 的 key 不匹配
            key = task_num
            try:
                import numpy as np
                if isinstance(key, np.generic):   # np.float32(1.0) -> 1.0
                    key = key.item()
                if isinstance(key, float) and key.is_integer():  # 1.0 -> 1
                    key = int(key)
            except Exception:
                pass

            val = real_param_dict.get(key, None)
            if val is None:
                val = real_param_dict.get(str(key), None)
            if val is None:
                val = 0.0  # 兜底：没有对应真实参数就用 0 着色，保证不崩
            plt.scatter(x, y, c=to_scalar(val) * np.ones_like(x), cmap=cmap, norm=norm)
    plt.colorbar()
