...
class EnvWorker:
    def __init__(self, parameter: Parameter, env_name='Hopper-v2', seed=0, policy_type=Policy,
        ...
        action_history_length = getattr(parameter, 'action_history_length', 1)
        self.action_history_length = action_history_length  # 保存以便后续使用

        self.ep_state_history_length = getattr(parameter, 'ep_state_history_length', 8)
        self.ep_action_window_length = getattr(parameter, 'ep_action_window_length', 10)
        self.history_constructor = HistoryConstructor(history_len, self.observation_space.shape[0],
                                                      ...
                                                      ep_state_history_length=self.ep_state_history_length,
                                                      ep_action_window_length=self.ep_action_window_length)
        self.env_tasks = None
        self.task_ind = -1
        self.env.reset()
        ...
        self.ep_cumrew_list = []
        self.ep_rew_list = []
        # 初始化 env_param_vector 为空数组（在 reset 中会被正确设置）
        self.env_param_vector = np.array([], dtype=np.float32)
    ...
    def set_global_seed(self, seed):
        ...
        import torch
        import random
        # 尝试调用环境的seed方法，如果不存在则跳过
        if hasattr(self.env, 'seed') and callable(getattr(self.env, 'seed', None)):
            try:
                self.env.seed(seed)
            except AttributeError:
                pass  # 某些环境可能没有实现seed方法
        np.random.seed(seed)
    ...
    def change_env_param(self, set_env_ind=None):
        # 检查是否是延迟环境
        is_delay_env = hasattr(self.env, '_delay_tasks') and hasattr(self.env, 'set_delay_task')
        ...
        if is_delay_env:
            ...
            if delay_tasks and len(delay_tasks) > 0:
                ...
                if self.non_stationary:
                    # 设置延迟非平稳参数
                    delay_changing_period = getattr(self.env, '_delay_changing_period', NON_STATIONARY_PERIOD)
                    delay_changing_interval = getattr(self.env, '_delay_changing_interval', NON_STATIONARY_INTERVAL)
                    delay_type_changing_period = getattr(self.env, '_delay_type_changing_period', None)
                    self.env.set_nonstationary_delay_para(
                        delay_tasks, delay_changing_period, delay_changing_interval,
                        delay_type_changing_period=delay_type_changing_period
                    )
        else:
            # 物理参数非平稳环境的处理逻辑（原有逻辑）
        ...
        with torch.no_grad():
            while step_count < min_batch:
                ...
                while True:
                    ...
                    step_result = self.env.step(self.env.denormalization(action))
                    # 关键：每一步都刷新 env_param_vector，保证 batch 内每条 transition
                    # 对应“当时刻”的非平稳延迟分布（而不是 reset 时的分布）。
                    # 这对 WMCL（用延迟分布监督 embedding）是必要的。
                    self.env_param_vector = self.env.env_parameter_vector
                    # 处理新的gymnasium API，step可能返回5个值(state, reward, terminated, truncated, info)
                    ...
                    if SKIP_MAX_LEN_DONE and done and self.env._elapsed_steps >= self.env._max_episode_steps:
                        mask = 1.0
                    action_history_size = self.ep_action_window_length * self.action_space.shape[0]
                    # last_action 统一为 (a_t, a_{t-1}, ..., a_{t-T+1})
                    # state 的前缀原本是 (a_{t-1},...,a_{t-T})，这里把当前 action 拼到最前面并丢掉最旧的
                    last_action_window = np.concatenate(
                        (action.astype(state.dtype).reshape(-1), state[:action_history_size - self.action_space.shape[0]]),
                        axis=-1,
                    )
                    mem.push(state[action_history_size:], action.astype(state.dtype), [mask],
                             next_state[action_history_size:], [reward],
                             None, [self.task_ind + 1], self.env_param_vector,
                             last_action_window, [done], [1])
    ...
    def reset(self, env_ind=None):
        ...
        # 处理新的gymnasium API，reset可能返回(observation, info)
...
class EnvRemoteArray:
    def __init__(self, parameter, env_name, worker_num=2, seed=None,
        ...
        #     use_remote = False
        # 获取动作历史长度
        self.action_history_length = getattr(parameter, 'action_history_length', 1)
        self.ep_state_history_length = getattr(parameter, 'ep_state_history_length', 8)
        self.ep_action_window_length = getattr(parameter, 'ep_action_window_length', 10)
    ...
    def sample1step(self, policy=None, random=False, device=torch.device('cpu'), env_ind=None):
        ...
        for ind, (next_state, reward, done, _, task_ind, env_param, current_steps) in enumerate(srd):
            ...
            # last_action 统一为 (a_t, a_{t-1}, ..., a_{t-T+1})
            last_action_window = np.concatenate(
                (actions[ind].astype(states.dtype).reshape(-1), states[ind, :action_history_size - self.action_space.shape[0]]),
                axis=-1,
            )
            mem.push(states[ind, action_history_size:], actions[ind].astype(states.dtype), [mask],
                     next_state[action_history_size:], [reward], None, [task_ind + 1],
                     env_param, last_action_window, [done], [1])
        if self.use_remote:
    ...
    def sample1step1env(self, policy, random=False, device=torch.device('cpu'), env_ind=None, render=False, need_info=False):
        ...
        if SKIP_MAX_LEN_DONE and done and current_steps >= self.env._max_episode_steps:
            mask = 1.0
        action_history_size = self.ep_action_window_length * self.act_dim
        # last_action 统一为 (a_t, a_{t-1}, ..., a_{t-T+1})
        last_action_window = np.concatenate(
            (action.astype(state.dtype).reshape(-1), state[:action_history_size - self.act_dim]),
            axis=-1,
        )
        mem.push(state[action_history_size:], action.astype(state.dtype), [mask],
                 next_state[action_history_size:], [reward], None,
                 [task_ind + 1], env_param, last_action_window, [done], [1])
