...
def compute_wasserstein_distance_1d_batched(probs):
    """
    批量计算一维离散分布之间的Wasserstein-1距离矩阵。

    参数:
        probs: [B, K] 每行是一个延迟分布的概率向量，所有行使用同一组支撑点
               (索引 0..K-1)

    返回:
        W: [B, B] Wasserstein-1距离矩阵
    """
    eps = 1e-8
    probs = probs / (probs.sum(dim=-1, keepdim=True) + eps)
    cdf = torch.cumsum(probs, dim=-1)  # [B, K]
    diff = cdf.unsqueeze(0) - cdf.unsqueeze(1)  # [B, B, K]
    W = diff.abs().sum(dim=-1)
    return W


class WassersteinMetricLoss:
    """
    Wasserstein Metric Context Loss (WMCL)

    目标：让上下文编码器的embedding距离与真实延迟分布的Wasserstein距离对齐。
    L_WMCL = 1/(B*(B-1)) * sum_{b!=b'} (D~z_{bb'} - D~q_{bb'})^2
    其中 D~ 表示归一化后的距离。
    """

    def __init__(self, max_dist_dim=11, min_pairs=2):
        self.max_dist_dim = max_dist_dim
        self.min_pairs = min_pairs

    def compute_loss(self, ep_embeddings, delay_distributions, valid=None):
        """
        参数:
            ep_embeddings: EP输出 [B, ep_dim] 或 [B, T, ep_dim]
            delay_distributions: 当前样本对应的延迟分布概率向量
                                  [B, K] 或 [B, T, K]
            valid: 有效性mask [B, 1] 或 [B, T, 1]，可选

        返回:
            标量 loss tensor，若不满足计算条件则返回 None
        """
        if ep_embeddings is None or delay_distributions is None:
            return None

        if ep_embeddings.dim() == 3:
            ep = ep_embeddings[:, -1, :]
        else:
            ep = ep_embeddings

        if delay_distributions.dim() == 3:
            q = delay_distributions[:, -1, :]
        else:
            q = delay_distributions
        ...
        if valid is not None:
            if valid.dim() == 3:
                ...
                v_mask = valid
            valid_idx = torch.where(v_mask > 0)[0]
            if valid_idx.numel() < self.min_pairs:
                return None
            ep = ep[valid_idx]
            q = q[valid_idx]

        prob_sum = q.sum(dim=-1)
        valid_q = prob_sum > 1e-6
        if valid_q.sum().item() < self.min_pairs:
            return None
        ep = ep[valid_q]
        q = q[valid_q]
        ...
        D_q = compute_wasserstein_distance_1d_batched(q).detach()
        D_z = torch.cdist(ep.unsqueeze(0), ep.unsqueeze(0), p=2).squeeze(0)

        off_diag = ~torch.eye(B, dtype=torch.bool, device=ep.device)

        D_q_off = D_q[off_diag]
        D_z_off = D_z[off_diag]

        D_q_mean = D_q_off.mean().detach() + 1e-8
        D_z_mean = D_z_off.mean() + 1e-8

        D_q_norm = D_q_off / D_q_mean
        D_z_norm = D_z_off / D_z_mean

        loss = (D_z_norm - D_q_norm).pow(2).mean()
        return loss
