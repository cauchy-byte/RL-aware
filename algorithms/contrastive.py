...
class ContrastiveLoss:
    def __init__(self, c_dim, max_env_len=40, ep=None):
        ...
        self.query_ep = ep
        self.c_dim = c_dim
        self.W = torch.nn.Parameter(torch.rand((c_dim, c_dim)))
        self.w_lr = 1e-1
        self.w_optim = torch.optim.Adam([self.W], lr=self.w_lr)
        self.loss_func = torch.nn.CrossEntropyLoss()

    def to(self, *arg, **kwargs):
        self.query_ep.to(*arg, **kwargs)
        self.W = torch.nn.Parameter(self.W.detach().to(*arg, **kwargs))
        self.w_optim = torch.optim.Adam([self.W], lr=self.w_lr)
