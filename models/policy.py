...
class Policy(torch.nn.Module):
    def __init__(self, obs_dim, act_dim, up_hidden_size, up_activations, up_layer_type,
                 ...
                 bottle_neck=False, bottle_sigma=1e-4, action_history_length=1,
        ...
        self.ep_temp = RNNBase(obs_dim + act_dim * ep_action_window_length, ep_dim, ep_hidden_size, ep_activation, ep_layer_type, logger)
    ...
    def tmp_ep_res(self, x, lst_a, h):
        ...
        参数:
            x: 状态 [batch, obs_dim]
            lst_a: 最近H个动作 [batch, act_dim * action_history_length]
            h: RNN隐藏状态
        """
    ...
    def meta_forward(self, x, lst_a, h, require_full_output=False):
        ...
        参数:
            ...
            h: RNN隐藏状态
        """
        #print(f"[DEBUG meta_forward] x shape: {x.shape}, lst_a shape: {lst_a.shape}")
        ep_h = h[:self.ep_rnn_count]
        up_h = h[self.ep_rnn_count:]
        if not require_full_output:
            if not self.use_gt_env_feature:
                # EP模块输入：状态 + 最近H个动作
                ep_input = torch.cat((x, lst_a), -1)
                #print(f"[DEBUG meta_forward] EP input shape: {ep_input.shape}, expected: {self.obs_dim + self.act_dim * self.action_history_length}")
                ep, ep_h_out = self.get_ep(ep_input, ep_h)
                ...
                # UP模块输入：状态 + EP输出
                #print(f"[DEBUG meta_forward] Before UP - x shape: {x.shape}, ep shape: {ep.shape}, ep_dim: {self.ep_dim}")
                up_input = torch.cat((x, ep), -1)
                #print(f"[DEBUG meta_forward] UP input shape: {up_input.shape}, UP expected input: {self.up.input_size}")
                up, up_h_out = self.up.meta_forward(up_input, up_h, aux_state=aux_input)
            else:
    ...
    def make_config_from_param(parameter):
        return dict(
            ...
            ep_state_history_length=getattr(parameter, 'ep_state_history_length', 8),
            ep_action_window_length=getattr(parameter, 'ep_action_window_length', 10)
        )
    ...
    def _inference_one_step_single(self, state, deterministic=True):
        with torch.no_grad():
            ...
            #print(f"[DEBUG inference_one_step] act_dim: {self.act_dim}, action_history_length: {self.action_history_length}")
            #print(f"[DEBUG inference_one_step] Expected action history size: {self.act_dim * self.action_history_length}")
            
            # 提取动作窗口（长度T）
            lst_action = state[..., :self.act_dim * self.ep_action_window_length]
            # 提取状态（在动作历史之后）
            state = state[..., self.act_dim * self.ep_action_window_length:]
            
            #print(f"[DEBUG inference_one_step] After split - state shape: {state.shape}, lst_action shape: {lst_action.shape}")
